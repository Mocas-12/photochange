...
(function () {
  ...
  function frame(t) {
    ...
    for (var i = 0; i < dots.length; i++) {
      ...
      var alpha = 0.07 + 0.045 * (wave * 0.5 + 0.5) + f * 0.6;
      if (alpha <= 0.02) continue;
      var size = 1.1 + f * 1.6;
      if (f > 0.03) {
        // 受亮点：向亮蓝提纯，保持单一色相
        var g = Math.round(163 + 42 * f);
        var r = Math.round(85 + 65 * f);
        ctx.fillStyle = 'rgba(' + r + ',' + g + ',255,' + Math.min(alpha, 0.9).toFixed(3) + ')';
        ctx.beginPath();

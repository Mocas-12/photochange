...
- 💛 **开发者支持模式**：完全免费、无导出次数限制、无功能锁，无需账号——觉得好用可以请作者喝杯咖啡
...
```text
photochange/
├── index.html          # 单页结构：工作台 + 自定义提示弹层（无内联脚本，CSP 约束）
├── style.css           # 深空辉光主题（玻璃拟态 + 氛围光晕 + 自托管 Inter 字体）
...
├── manifest.webmanifest / service-worker.js  # PWA：可安装 + 离线外壳（预缓存清单自动生成）
├── vendor/             # 自托管第三方库（jsPDF、heic2any，均按需加载）
├── tests/              # Playwright 测试（npm test）
└── wechatpay/
    └── wechatpay.jpg   # 收款码图片
```
...
- **完全免费**：所有功能无限制使用，没有导出次数上限，没有功能锁，也不需要注册账号
- **没有付费墙**：不存在激活码或专业版；如果此前的说明让你以为需要付费，那是文档没跟上，功能本身一直是全量本地的
- **支持是可选的**：如果这个工具帮到了你，欢迎[请作者喝杯咖啡](https://mbd.pub/o/bread/mbd-YZWblJ9paA==)（页面底部导出区也有入口），但这纯粹是自愿的感谢，不影响任何功能
- 收到的支持将用于域名、CDN 与继续开发
...
本项目基于 [MIT](./LICENSE) 许可证开源——可自由使用、修改与再分发（含商用），只需在副本中保留版权与许可声明。
